...
def test_predict_endpoint_preserves_single_input_shape(
    ...
    response = predict_churn(feature_vector, trained_artifact)
...
def test_predict_endpoint_preserves_batch_order(
    ...
    batch_response = predict_churn([first, second], trained_artifact)
    first_response = predict_churn(first, trained_artifact)
    second_response = predict_churn(second, trained_artifact)
...
def test_predict_openapi_contains_single_and_batch_examples() -> None:
    operation = create_app().openapi()["paths"]["/predict"]["post"]
